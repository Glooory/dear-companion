...
describe("SettingsStore", () => {
  ...
  it("leaves the primary untouched when its atomic replacement fails", async () => {
    ...
    ioFaults.failedAtomicWritePaths.clear();
    expect(await store.load()).toEqual(existing);
    expect(JSON.parse(await readFile(join(userDataPath, "settings.backup.json"), "utf8"))).toEqual(existing);
  });
  ...
  it("leaves recovery files untouched when restoring the backup fails", async () => {
    ...
    expect(JSON.parse(await readFile(join(userDataPath, "settings.backup.json"), "utf8"))).toEqual(previous);
  });
  ...
  it("treats an old schema version as invalid rather than rewriting it", async () => {
    const userDataPath = await createUserDataPath();
    const legacy = {
      schemaVersion: 4,
      activePetId: null,
