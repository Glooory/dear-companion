...
function harness() {
  const points = [{ x: 80, y: 100 }, { x: 105, y: 100 }, { x: 80, y: 100 }, { x: 105, y: 100 }]
  let pointIndex = 0
  let now = 0
  ...
  const tracker = new PettingTracker({
    ...
    onDetected: detected, now: () => now,
    setInterval: (callback) => {
      const timer = { callback, cleared: false }; intervals.push(timer)
      return timer as unknown as ReturnType<typeof setInterval>
    },
    clearInterval: (timer) => { (timer as unknown as { cleared: boolean }).cleared = true },
    setTimeout: (callback) => {
      const timer = { callback, cleared: false }; timeouts.push(timer)
      return timer as unknown as ReturnType<typeof setTimeout>
    },
    clearTimeout: (timer) => { (timer as unknown as { cleared: boolean }).cleared = true }
  })
  return { tracker, intervals, timeouts, detected, advance(ms = 100) { now += ms } }
}
...
describe('PettingTracker', () => {
  it('samples only while armed and emits once', () => {
    ...
    for (let count = 0; count < 3; count += 1) { h.advance(); timer.callback() }
