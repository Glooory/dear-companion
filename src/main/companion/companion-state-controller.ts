...
export class CompanionStateController {
  ...
  async refresh(): Promise<void> {
    ...
    if (
      (this.manualSelection === "drowsy" && !available.drowsy) ||
      (this.manualSelection === "sleeping" && !available.sleeping)
    ) {
      this.manualSelection = "auto";
    }
    if (
      (this.automaticState === "drowsy" && !available.drowsy) ||
  ...
  wakeFromSleep(): void {
    if (
      ...
      this.manualWorkActive ||
      this.scheduledWorkActive ||
  ...
  private recompute(publish: boolean): void {
    ...
    this.scheduledWorkActive = this.settings.workSchedules.some((schedule) => isWorkScheduleActive(schedule, now));
  ...
  private handleTimer(expected: number, rhythmBoundary: number | null): void {
    ...
    const clockChanged =
      Math.abs(elapsedWall - elapsedMonotonic) > CLOCK_CHANGE_TOLERANCE_MS ||
