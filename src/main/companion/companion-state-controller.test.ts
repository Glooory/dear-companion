...
function pet(): PetConfig {
  ...
  return {
    ...
    lifeStates: {
      ...
      sleeping: { enabled: true, assetIds: ["sleeping"] },
      workingAssetIds: [],
    },
    companionPace: "natural",
    interactionBubblesEnabled: true,
    dialogueSettings: { address: "", voiceEnabled: false, voiceVolume: 0.8, categories: {}, quickDialogueRefs: [] },
  };
}
...
function harness(initial = settings()) {
  ...
  return {
    ...
    setTimezoneOffset(value: number) {
...
describe("CompanionStateController", () => {
  it("starts calm, replaces timers, and respects manual state availability", async () => {
    ...
    h.setSettings(
      settings({
        pets: [{ ...pet(), lifeStates: { ...pet().lifeStates, sleeping: { enabled: false, assetIds: ["sleeping"] } } }],
      })
    );
