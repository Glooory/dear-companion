...
export class ReminderScheduler {
  ...
  private wake(): void {
    ...
    const clockChanged = Math.abs(elapsedWall - elapsedMonotonic) > 2_000 || this.timezoneOffset() !== this.lastTimezoneOffset
  ...
  private enqueueOccurrence(occurrence: ReminderOccurrence): void {
    if (this.activePrompt?.occurrenceId === occurrence.occurrenceId || this.queue.some((item) => item.occurrenceId === occurrence.occurrenceId)) return
  ...
  private scheduleWake(now: number): void {
    ...
    const nextSnooze = [...this.snoozes.values()].reduce<number | null>((nearest, entry) => nearest === null ? entry.dueAt : Math.min(nearest, entry.dueAt), null)
    const nextDue = Math.min(nextOccurrence?.scheduledFor ?? Number.POSITIVE_INFINITY, nextSnooze ?? Number.POSITIVE_INFINITY)
