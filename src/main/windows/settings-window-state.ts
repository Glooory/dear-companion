...
export async function loadSettingsWindowState(userDataPath?: string): Promise<SettingsWindowBounds | null> {
...
export async function saveSettingsWindowState(
  ...
  if (!validated) return;
  try {
    await writeFileAtomic(join(userDataPath, SETTINGS_WINDOW_STATE_FILE), JSON.stringify(validated, null, 2), {
      mode: 0o600,
    });
  } catch {
