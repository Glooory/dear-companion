...
export function registerRestSystemIpc(dependencies: Dependencies): () => void {
  ...
  const requireSettings = (senderId: number): void => {
    if (dependencies.windowManager.getWindowKind(senderId) !== 'settings') throw new Error('This operation is available only from settings')
  }
  const requirePet = (senderId: number): void => {
    if (dependencies.windowManager.getWindowKind(senderId) !== 'pet') throw new Error('This operation is available only from the pet window')
  }
  const snapshot = async (): Promise<RestSystemSnapshot> => createRestSystemSnapshot(
    await dependencies.settingsStore.load(), dependencies.getRuntimeSnapshot()
  )
  ...
  const playbackFailureListener = (event: IpcMainEvent, requestId: unknown, assetId: unknown): void => {
    try {
      ...
      void dependencies.audioService.reportPlaybackFailure(requestId, assetId).then(() => broadcast()).catch(() => undefined)
    } catch {
  ...
  try {
    ...
    handle(IPC_CHANNELS.setReminderEnabled, async (event, idValue: unknown, enabled: unknown) => {
      ...
      await dependencies.settingsStore.update((current) => {
        if (!current.reminders.some((item) => item.id === id)) throw new Error('Reminder does not exist')
        return { ...current, reminders: current.reminders.map((item) => item.id === id ? { ...item, enabled } : item) }
      })
      return refreshAndBroadcast()
    })
    ...
    handle(IPC_CHANNELS.importAudio, async (event) => {
      ...
      const selection = await dialog.showOpenDialog(owner, {
        title: '导入本地音频', properties: ['openFile', 'multiSelections'],
