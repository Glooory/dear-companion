...
import { parseAutostartEnabledInput, parseAutostartStatus, parsePetRendererStatus } from "../../shared/contracts";
...
interface ReleaseHardeningIpcDependencies {
  ...
  windowManager: Pick<WindowManager, "getWindowKind" | "getPetRendererStatus" | "retryPetRenderer">;
}
...
export function registerReleaseHardeningIpc({
  ...
  ipcMain.handle(IPC_CHANNELS.setAutostartEnabled, async (event, enabled: unknown) => {
    ...
    return parseAutostartStatus(await autostartService.setEnabled(parseAutostartEnabledInput(enabled)));
  });
