...
describe('image input validation', () => {
  ...
  it('rejects corrupt dimensions, oversized dimensions, opacity, and empty alpha', () => {
    ...
    expectCode(() => validateDecodedImage({ width: 8193, height: 1, bitmap: new Uint8Array() }), 'dimensions-too-large')
    expectCode(() => validateDecodedImage({ width: 1, height: 1, bitmap: new Uint8Array([0, 0, 0, 255]) }), 'no-transparency')
    expectCode(() => validateDecodedImage({ width: 1, height: 1, bitmap: new Uint8Array([0, 0, 0, 0]) }), 'fully-transparent')
  })
