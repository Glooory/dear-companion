...
function electronApp(loginItemSettings: LoginItemSettings): Pick<
  App,
  'getLoginItemSettings' | 'setLoginItemSettings' | 'isPackaged'
> {
  return {
    isPackaged: true,
...
describe('AutostartService effective state', () => {
  ...
  it('persists an external Windows disable without re-registering the app', async () => {
    ...
    const app = electronApp(loginSettings({
      openAtLogin: true,
      launchItems: [{
        name: 'Dear Companion',
        path: executablePath,
