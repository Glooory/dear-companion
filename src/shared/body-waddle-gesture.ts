...
export class BodyWaddleGesture {
  ...
  register(sample: BodyPointerSample, region: BodyWaddleRegion, pettingCandidateActive = false): HorizontalDirection {
    ...
    if (Math.abs(this.accumulatedX) < this.threshold || Math.abs(this.accumulatedX) < Math.abs(this.accumulatedY)) {
      return 0;
    }
...
function isInBodyRegion(sample: BodyPointerSample, region: BodyWaddleRegion): boolean {
  ...
  if (
    ...
    sample.x < visibleRect.x ||
    sample.x > visibleRect.x + visibleRect.width ||
    sample.y < visibleRect.y ||
    sample.y > visibleRect.y + visibleRect.height
  ) {
    return false;
  }
