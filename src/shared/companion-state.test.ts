...
const base: CompanionStateInputs = {
  current: 'daily-calm', automaticState: 'daily-playful', manualSelection: 'auto',
...
describe('resolveCompanionState', () => {
  it('applies system, work, manual and automatic priority', () => {
    expect(resolveCompanionState({ ...base, current: 'sleeping', systemSuspended: true, manualWorkActive: true })).toBe('sleeping')
  ...
  it('falls back safely when an optional state is unavailable', () => {
    expect(resolveCompanionState({ ...base, manualSelection: 'sleeping', available: { drowsy: true, sleeping: false } })).toBe('daily-calm')
    expect(resolveCompanionState({ ...base, automaticState: 'drowsy', available: { drowsy: false, sleeping: true } })).toBe('daily-calm')
  })
})
