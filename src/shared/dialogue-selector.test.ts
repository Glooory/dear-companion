...
describe("DialogueSelector", () => {
  it("enforces category cooldowns and excludes the last two lines", () => {
    ...
    expect(selector.select({ category: "daily:click", lines, now: 2_999, random: () => 0 })).toBeNull();
    expect(selector.select({ category: "daily:click", lines, now: 3_000, random: () => 0 })).toBe("b");
    expect(selector.select({ category: "daily:petting", lines, now: 3_001, random: () => 0 })).toBe("c");
  });
