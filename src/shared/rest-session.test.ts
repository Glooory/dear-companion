...
function session(): RestSessionSnapshot {
  return {
    sessionId: 'session-1', scheduleId: 'reminder-1', startedAt: 1_000, endsAt: 11_000,
    state: 'resting', cryingUntil: null, cursorTolerance: 'standard', message: '休息',
...
describe('rest session calculations', () => {
  ...
  it('completes late ticks and resumes according to endsAt', () => {
    ...
    expect(transitionRestSession(session(), { type: 'resume' }, 5_000)).toMatchObject({ state: 'resting', endsAt: 11_000 })
  ...
  it('ends manually and clears after celebration', () => {
    ...
    expect(transitionRestSession({ ...session(), state: 'celebrating' }, { type: 'celebration-complete' }, 12_000)).toBeNull()
  })
})
