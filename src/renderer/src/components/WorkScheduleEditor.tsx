...
interface Draft extends CreateWorkScheduleInput {
  id?: string;
}
        ...
        <div className={styles.editor}>
          <div className={styles.timeFields}>
            <TimeField
              ...
              onChange={(hour, minute) => setDraft({ ...draft, startHour: hour, startMinute: minute })}
            />
            <TimeField
              label="结束"
              hour={draft.endHour}
              minute={draft.endMinute}
              onChange={(hour, minute) => setDraft({ ...draft, endHour: hour, endMinute: minute })}
            />
          </div>
          <WeekdayPicker
          ...
          <div className="editor-actions">
            ...
            <button type="button" className="secondary-button" disabled={disabled} onClick={() => setDraft(null)}>
              取消
            </button>
            <button
              type="button"
          ...
          {schedules.map((schedule) => (
            <div className={styles.row} key={schedule.id}>
              <button
                ...
                onClick={() => setDraft({ ...schedule, weekdays: [...schedule.weekdays] })}
              >
                <strong>
