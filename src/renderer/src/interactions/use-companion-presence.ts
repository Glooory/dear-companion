...
export function useCompanionPresence(options: UseCompanionPresenceOptions): void {
  ...
  useEffect(() => {
    ...
    const wait = options.busy && dueAt <= now
      ? 1_000 + Math.round(Math.random() * 2_000)
    ...
    const timer = window.setTimeout(() => {
      ...
      if (layer === 'ambient') deadline.consume(firedAt, nextAmbientDelay(options.lifeState === 'working' ? 'quiet' : options.pace, Math.random))
