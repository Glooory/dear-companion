...
export function useDialogue(
  ...
  const show = useCallback(
    (
      ...
      const required = Array.isArray(linesOrRequired)
        ? Boolean(maybeRequired)
        : Boolean(linesOrRequired)
      ...
      if (Array.isArray(linesOrRequired)) {
        ...
        lines = SYSTEM_DIALOGUES.crying.map((line) =>
          line.replaceAll(ADDRESS_PLACEHOLDER, address)
        )
      } else if (category === 'system:completion') {
        const address = dialogueSettings?.address.trim() ?? ''
        lines = SYSTEM_DIALOGUES.reminderCompletion.map((line) =>
          line.replaceAll(ADDRESS_PLACEHOLDER, address)
        )
      } else {
        lines = []
